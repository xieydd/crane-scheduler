...
const (
	...
	// max number of points each time series of metrics
...
// QueryRangeSync range query metrics in sync way
...
// QuerySync query metrics in sync way
...
func (c *context) convertPromResultsToTimeSeries(value prommodel.Value) ([]*common.TimeSeries, error) {
	...
	case prommodel.ValMatrix:
		if matrix, ok := value.(prommodel.Matrix); ok {
			...
			return results, fmt.Errorf("metrics value type is %v, but assert failed", typeValue)
		}
	...
	case prommodel.ValNone:
		return results, fmt.Errorf("metrics return value type is none")
	}
	return results, fmt.Errorf("metrics return unknown model value type %v", typeValue)
}
