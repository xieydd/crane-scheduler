...
// NewPrometheusClient returns a metrics.Client
...
// prometheusAuthClient wraps the metrics api raw client with authentication info
...
// URL implements metrics client interface
func (pc *prometheusAuthClient) URL(ep string, args map[string]string) *url.URL {
	return pc.client.URL(ep, args)
}

// Do implements metrics client interface, wrapped with an auth info
...
func (pc *prometheusRateLimitClient) Do(ctx gocontext.Context, req *http.Request) (*http.Response, []byte, error) {
	...
	// block wait until at least one InFlight request finished if current inflighting requests reach the max limit, avoid many time consuming requests hit the metrics.
	// we use inflight to record the number of inflighting requests, because metrics query is time-consuming when the range is large
