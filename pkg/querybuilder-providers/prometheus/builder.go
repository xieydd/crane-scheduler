...
// todo: later we change these templates to configurable like metrics-adapter
const (
	...
	// NodeCpuUsageExprTemplate is used to query node cpu usage by promql,  param is node name,common condition,node name, common condition, which metrics scrape, duration str
	...
	// NodeMemUsageExprTemplate is used to query node cpu memory by promql,  param is node name, node name which metrics scrape
	...
	// ContainerMemUsageExprTemplate is used to query container cpu usage by promql,  param is namespace,workload,container
...
func (b *builder) workloadQuery(metric *metricquery.Metric, behavior querybuilder.BuildQueryBehavior) (*metricquery.Query, error) {
	...
	if selector != nil && behavior.FederatedClusterScope {
		if id, exists := selector.RequiresExactMatch(known.LabelClusterId); exists {
