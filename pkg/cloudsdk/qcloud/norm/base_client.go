...
// DoRequest request an interfaceName
func (c *httpClient) DoRequest(interfaceName string, request interface{}, response interface{}) (responseErr error) {
	...
	klog.V(6).Info("Http request to: ", c.url, ", provider: ", interfaceName, ", request: ", string(requestData))
	...
	if err != nil {
		msg := "DoRequest failed!read http response failed:" + err.Error() + ", url: " + c.url + ", provider: " + interfaceName
		klog.Error(msg)
	...
	klog.V(6).Info("Http response from: ", c.url, ", provider: ", interfaceName, ", response: ", string(body))
	...
	if code != 0 {
		...
		klog.Error("interfaceName:", interfaceName, " c.getResult code: ", code, "msg:", msg)
		return
	}
	return nil
}
